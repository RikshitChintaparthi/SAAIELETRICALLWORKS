...
export default function CompanyList({ companies, onEdit, onDelete, onReorder, token }: CompanyListProps) {
  ...
  const handleDragEnd = async () => {
    ...
    if (onReorder && token) {
      onReorder(items)
      try {
        await fetch("/api/reorder/companies", {
          method: "POST",
          headers: {
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <div className="flex gap-4 mt-2 text-sm text-slate-500">
                  <span>Website: {company.website || "N/A"}</span>
